...
![Demo](https://img.shields.io/badge/C%2B%2B-17-blue) ![Platforms](https://img.shields.io/badge/Platform-Linux%20|%20macOS%20|%20Windows-green)
...
1. **读取输入**：持续读取整数，直到遇到空行  
2. **提取参数**：最后一个数作为分组宽度（`groupDiffLength`）  
3. **分组统计**：
   - 计算数据最小值和最大值
   - 按指定宽度划分区间（bins）
   - 统计每个区间的频次
...
👉 [下载最新版本](https://github.com/your-username/histogram/releases)  
*(请将 `your-username` 替换为实际仓库所有者)*
